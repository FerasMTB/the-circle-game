...
export default function WinningModal({ isOpen, prize, onClose }: WinningModalProps) {
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            const t = setTimeout(() => setShow(false), 300); // Wait for exit animation
    ...
    return (
        ...
        >
            <div
                className={`relative w-full max-w-md transform overflow-hidden rounded-2xl bg-gradient-to-b from-slate-900 to-black p-1 text-center shadow-2xl ring-1 ring-[#D4AF37] transition-all duration-500 ${isOpen ? "scale-100 translate-y-0" : "scale-95 translate-y-8"
                    }`}
            >
